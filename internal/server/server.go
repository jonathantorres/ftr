...
func (s *Server) Start() error {
	...
	s.LogAcc("server started OK, waiting for connections")
...
func (s *Server) Shutdown() error {
	...
	if s.listener != nil {
		...
		if err != nil {
			s.LogErr("error closing the main listener: %s", err)
		}
	}

	s.LogAcc("server shutdown complete")
	s.shutdownC <- struct{}{}
...
func (s *Server) Reload() error {
	s.LogAcc("reloading configuration file...")
	_, err := conf.Load(Prefix+DefaultConf, Prefix)
	if err != nil {
		s.LogErr("configuration file error: %s", err)
		return err
	}

	s.LogAcc("configuration file OK.")
	s.IsReloading = true
	s.Shutdown()
...
func (s *Server) LogErr(fmt string, v ...interface{}) {
	s.ErrorLog.Printf(fmt, v...)
}

func (s *Server) LogAcc(fmt string, v ...interface{}) {
	s.AccessLog.Printf(fmt, v...)
}
...
func (s *Server) handleClient(conn *net.TCPConn) {
	if s.isShuttingDown {
		err := conn.Close()
		if err != nil {
			s.LogErr("error closing accepted connection on shutdown: %s", err)
		}
